...
# --- LAYOUT PRINCIPAL EN TRES MÓDULOS (PESTAÑAS) ---
tab_chat, tab_equipo, tab_regimen = st.tabs([
    "💬 Agente de Turnos",
    "📋 Plantilla de Operadores",
    "🗓️ Régimen de 5 Semanas",
])

with tab_equipo:
  st.subheader("Gestión de Personal y Posición de Prioridad")
  st.markdown(
  ...
  with col_tabla:
    ...
    for idx, op in enumerate(st.session_state.operadores):
      # Estructura compacta y discreta: [Info (ancho)] [▲] [▼] [✕]
      c_info, c_up, c_down, c_del = st.columns([5, 0.6, 0.6, 0.6])
      with c_info:
        roles_str = ", ".join(op["Roles Habilitados"])
        ciclo_val = op.get("Semana Ciclo", 1)
        st.markdown(
            f"**{idx+1}. {op['Nombre']}** — `[{roles_str}]` | Ciclo:"
            f" **Semana {ciclo_val}**"
        )
      with c_up:
        if idx > 0:
          if st.button("▲", key=f"up_{idx}", help="Subir posición"):
            (
      ...
      with c_down:
        if idx < len(st.session_state.operadores) - 1:
          if st.button("▼", key=f"down_{idx}", help="Bajar posición"):
            (
                st.session_state.operadores[idx],
                st.session_state.operadores[idx + 1],
            ) = (
            ...
            st.rerun()
      with c_del:
        if st.button("✕", key=f"del_{idx}", help="Eliminar operador"):
...
with tab_regimen:
  ...
  st.markdown(
      "Esta tabla define el patrón estándar de rotación de turnos aplicable de"
      " manera natural a los operadores según su semana de ciclo, con total"
  ...
  regimen_data = {
      "Semana del Ciclo": ["1 (OI)", "2 (T2)", "3 (T3)", "4 (T1)", "5 (D)"],
      "Lunes": ["OI", "T2", "T3", "T1", "D"],
    ...
    st.session_state.messages = [
        {
            "role": "assistant",
            "content": (
                "¡Hola! Soy tu agente experto en turnos. Tengo separada la"
                " gestión de personal, el régimen de 5 semanas en su propio"
                " módulo y los controles optimizados. ¿Qué deseas planificar o"
                " consultar hoy?"
            ),
        }
    ]
    ...
    if not client:
      ...
      with st.chat_message("assistant"):
        with st.spinner("El agente está procesando tu solicitud..."):
          try:
            lista_nombres_ops = "\n".join([
                f"- {op['Nombre']}: Semana de ciclo {op.get('Semana Ciclo', 1)}, Roles: {op['Roles Habilitados']}"
